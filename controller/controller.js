        ...
        bcrypt.compare(password, user.password, (bcryptErr, bcryptResult) => {
          ...
          if (bcryptResult) {
            const token = jwt.sign(
              { email: user.email, id: user.id, rol: user.rol },
              "YourSecretKey",
              {
                expiresIn: "3m",
              }
    ...
    db.query(checkUserQuery, [email], (error, results) => {
      if (error) {
        ...
        return res
          ...
          .json({
            success: false,
            message: "Error en la verificación de usuario existente",
            error: error.message,
          });
      }
      ...
      if (results.length > 0) {
        return res
          ...
          .json({
            success: false,
            message:
              "Usuario existente. Por favor, elija otro correo electrónico.",
          });
      }
      ...
      const insertUserQuery =
        "INSERT INTO empleados (nombre, apellido, email, password, telefono, valor_hora, rol, edad, pais, experiencia) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
      const values = [
        data.nombre,
        data.apellido,
        data.email,
    ...
    (err, result) => {
      if (err) {
        ...
        res
          ...
          .json({
            success: false,
            message: "Error en la creación del empleado",
            error: err.message,
          });
      } else {
        res
    ...
    (err, result) => {
      if (err) {
        ...
        res
          ...
          .json({
            success: false,
            message: "Error en la actualización del empleado",
            error: err.message,
          });
      } else {
        res
...
const createText = (req, res) => {
  ...
  const insertTextQuery =
    "INSERT INTO informacion (nombre, fecha, texto) VALUES (?, ?, ?)";
  ...
  db.query(insertTextQuery, values, (error, result) => {
    if (error) {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Error en la inserción del texto" });
    }
    res
      .status(200)
      .json({ success: true, message: "Texto creado exitosamente" });
  });
};
    ...
    (err, result) => {
      if (err) {
        ...
        res
          ...
          .json({
            success: false,
            message: "Error en la actualización de info",
            error: err.message,
          });
      } else {
        res
